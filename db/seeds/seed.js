...
const seed = async (data) => {
  ...
  CREATE TABLE comments (
    ...
    const categoriesPromise = db.query(insertCategoriesQueryStr).then((result) => result.rows);
    ...
    const reviewRows = await db.query(insertReviewsQueryStr).then((result) => result.rows);
    ...
    const insertCommentsQueryStr = format(
        ...
        formattedCommentData.map(({ body, author, review_id, votes = 0, created_at }) => [
            body,
            author,
            review_id,
            votes,
